	...
	$('#vediodg').datagrid(
			{
				...
				onExpandRow : function(index, row) {
					...
					ddv.panel( {
						...
						onLoad : function() {
							$('#vediodg').datagrid('fixDetailRowHeight', index);
							$('#vediodg').datagrid('selectRow', index);
							$('#vediodg').datagrid('getRowDetail', index).find(
									'form').form('load', row);
						}
					});
					$('#vediodg').datagrid('fixDetailRowHeight', index);
...
function editAdver() {
	...
	var row = $('#vediodg').datagrid('getSelected');
	if (row) {
		$('#vediodlg').dialog('open').dialog('setTitle', '修改广告');
...
function advercancel(){
	$('#vediodlg').dialog({closed:true}); //关闭对话框
}
		...
		$.messager.confirm('Confirm','是否确定删除该条记录？',function (r){
			if(r){
				$.post("../AdService.do?",{
					...
					if(result){
						$('#vediodg').datagrid(queryAdverList());
					}else{
						$.messager.show({
							title:'出错啦!',
							msg:"删除失败"
